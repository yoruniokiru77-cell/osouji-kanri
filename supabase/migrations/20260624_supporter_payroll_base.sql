...
create or replace function public.replace_own_reservation_workers(
  ...
  target_support_worker_ids uuid[] default array[]::uuid[]
)
returns void
language plpgsql
security definer
...
declare
  ...
  normalized_support_worker_ids uuid[];
  valid_worker_count integer;
  valid_support_worker_count integer;
begin
  if not exists (
    select 1
  ...
  select array_agg(distinct selected.worker_id)
  into normalized_support_worker_ids
  from unnest(coalesce(target_support_worker_ids, array[]::uuid[])) as selected(worker_id)
  where selected.worker_id <> all(normalized_worker_ids);

  if coalesce(cardinality(normalized_support_worker_ids), 0) > 0 then
    select count(*)
    into valid_support_worker_count
    from public.workers w
    where w.id = any(normalized_support_worker_ids)
      and w.active;

    if valid_support_worker_count <> cardinality(normalized_support_worker_ids) then
      raise exception 'One or more selected supporters are invalid.';
    end if;
  end if;
  ...
  insert into public.reservation_workers (reservation_id, worker_id, is_supporter)
  select target_reservation_id, selected.worker_id, false
  from unnest(normalized_worker_ids) as selected(worker_id);

  insert into public.reservation_workers (reservation_id, worker_id, is_supporter)
  select target_reservation_id, selected.worker_id, true
  from unnest(coalesce(normalized_support_worker_ids, array[]::uuid[])) as selected(worker_id);
...
revoke all on function public.replace_own_reservation_workers(uuid, uuid[], jsonb, uuid[]) from public;
grant execute on function public.replace_own_reservation_workers(uuid, uuid[], jsonb, uuid[]) to authenticated;
