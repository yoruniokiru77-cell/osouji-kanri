...
- `ADMIN_HOSTS`（任意。管理者専用ドメインをカンマ区切りで指定。例: `admin-osouji-kanri.vercel.app`）
...
スタッフと管理者を同時にログインする場合は、Vercelで同じプロジェクトに管理者用の別ドメインを追加します。
例: `admin-osouji-kanri.vercel.app`

管理者用ドメインでは `/login` や `/staff/...` にアクセスしても管理者ログイン・管理画面へ誘導されます。
